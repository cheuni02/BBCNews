Feature: OSB Emissions Fix - Book service for RTC retailers (VWOSBEB-2)
  ...
  Note that in the OSB, the user will enter his Reg number - we will need to get the corresponding VIN in order to match it to the VINs list
...
Scenario: my car is on the affected VINs list - the emissions fix work should be available in the list FOC
  Given my car is on the affected cars list and i have entered the VIN on the first step
